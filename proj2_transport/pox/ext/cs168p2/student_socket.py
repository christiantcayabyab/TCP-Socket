...
class StudentUSocket(StudentUSocketBase):
  ...
  def connect(self, ip, port):
    ...
    p = self.new_packet(False, None, True)
      ...
      if self.acceptable_seg(seg, payload):
        ...
        if (self.rx_queue.peek()[0] |GT| self.rcv.nxt):
          ...
          while not (self.rx_queue.empty()):
            pop_tup = self.rx_queue.pop()
            if (self.rcv.nxt == pop_tup[0]):
              self.handle_accepted_seg(pop_tup[1].tcp, pop_tup[1].app)
  ...
  def update_rto(self, acked_pkt):
    ...
    if self.srtt == 0:
      ...
      self.rttvar = ((1.0 - self.beta) * self.rttvar) + (self.beta * abs(self.srtt - R_prime))
      self.srtt = ((1.0 - self.alpha) * self.srtt) + (self.alpha * R_prime)
      self.rto = self.srtt + max(self.G, self.K * self.rttvar)
  ...
  def handle_accepted_fin(self, seg):
    ...
    if self.state in (FIN_WAIT_2):
      #we receive a FIN while in FIN_WAIT_2
      if seg.FIN:
        self.start_timer_timewait()
  ...
  def maybe_send(self):
    ...
    while (len(self.tx_data) > 0):
      real_window = snd.wnd - (snd.nxt - snd.una)
      if real_window <= 0:
        break
      index = min(len(self.tx_data), real_window, mss)
      send_packet = self.new_packet(data=self.tx_data[:index])
      self.tx_data = self.tx_data[index:]
      self.tx(send_packet)
  ...
  def check_timer_retx(self):
    ...
    if (self.retx_queue.empty()):
      return
    time_in_queue = self.stack.now - self.retx_queue.peek()[1].tx_ts
